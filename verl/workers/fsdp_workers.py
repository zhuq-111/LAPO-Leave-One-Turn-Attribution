...
class ActorRolloutRefWorker(Worker):
    ...
    def compute_log_prob(self, data: DataProto) -> DataProto:
        ...
        with self.ulysses_sharding_manager:
            ...
            if isinstance(old_log_probs, dict):
                output = DataProto.from_dict(tensors=old_log_probs)
            else:
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            short_answer_limit = prompts.meta_info.get('max_new_tokens')
            generation_kwargs = {'max_tokens': int(short_answer_limit)} if short_answer_limit else {}
            output = self.rollout.generate_sequences(prompts=prompts, **generation_kwargs)
