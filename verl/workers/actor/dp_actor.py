...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, return_logits: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns: 
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if return_logits:
                    full_logits = pad_input(hidden_states=logits_rmpad,
                                            indices=indices,
                                            batch=batch_size,
                                            seqlen=seqlen)
                ...
                response_logits = logits[:, -response_length - 1:-1]  # (bsz, response_length)
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            with torch.no_grad():
                output = self._forward_micro_batch(micro_batch, temperature=temperature, return_logits=return_logits)
                if return_logits:
                    _, log_probs, logits = output
        ...
        logits = torch.concat(logits_lst, dim=0) if return_logits else None
        ...
        if return_logits:
            return {'old_log_probs': log_probs, 'logits': logits}
        if return_entropy:
            return {'old_log_probs': log_probs, 'entropy': entropy}
        return log_probs
