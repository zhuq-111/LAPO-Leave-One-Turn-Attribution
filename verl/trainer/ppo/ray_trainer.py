...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,
                      norm_adv_by_std_in_grpo=True, info_gain_norm_mode="joint", use_counterfactual_ig=False):
    ...
    if adv_estimator == 'gae':
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        ...
                                                                        use_counterfactual_ig=use_counterfactual_ig)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    else:
...
def _infer_eval_dataset_name(sample_id):
    sample_id = str(sample_id)
    if sample_id.startswith('nq_'):
        return 'nq'
    if sample_id.startswith('trivia_'):
        return 'trivia'
    if sample_id.startswith('hotpot_'):
        return 'hotpot'
    if sample_id.startswith('2wiki_') or sample_id.startswith('2wikimultihopqa_'):
        return '2wiki'
    return None
...
def compute_data_metrics(batch, use_critic=True):
    ...
    if 'valid_search_stats' in batch.meta_info:
        ...
        metrics['env/valid_search/mean'] = float(valid_search_stats.mean())
        metrics['env/valid_search/max'] = float(valid_search_stats.max())
        metrics['env/valid_search/min'] = float(valid_search_stats.min())
        if 'turns_stats' in batch.meta_info:
            turns_stats = np.array(batch.meta_info['turns_stats'], dtype=np.int16)
            metrics['env/ratio_of_valid_search'] = float((valid_search_stats / turns_stats).mean())
    if 'cf_ig_metrics' in batch.meta_info:
        metrics.update(batch.meta_info['cf_ig_metrics'])
    if valid_adv.numel() > 1:
        metrics['critic/advantage_std'] = torch.std(valid_adv).detach().item()
    else:
        metrics['critic/advantage_std'] = 0.0
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            log_prob_micro_batch_size=self.config.actor_rollout_ref.rollout.log_prob_micro_batch_size,
            log_prob_use_dynamic_bsz=self.config.actor_rollout_ref.rollout.log_prob_use_dynamic_bsz,
            log_prob_max_token_len_per_gpu=self.config.actor_rollout_ref.rollout.log_prob_max_token_len_per_gpu,
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                sample_id_lst.append(test_batch.non_tensor_batch.get('id', ['unknown'] * reward_tensor.shape[0]))
        else:
            for batch_dict in self.val_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        # Validation reports outcome reward only; skip info-gain probes.
                        final_gen_batch_output = generation_manager.run_llm_loop(
                            gen_batch=test_gen_batch,
                            initial_input_ids=first_input_ids,
                            ground_truths=None,
                        )
                        final_gen_batch_output.non_tensor_batch.pop('info_gain_rewards', None)
        ...
        sample_ids = np.concatenate(sample_id_lst, axis=0)
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            eval_dataset = _infer_eval_dataset_name(sample_ids[i])
            if eval_dataset is not None:
                if eval_dataset not in eval_dataset_reward:
                    eval_dataset_reward[eval_dataset] = []
                eval_dataset_reward[eval_dataset].append(reward_tensor[i].item())

        metric_dict = {}
        for data_source, rewards in data_source_reward.items():
            metric_name = 'total' if data_source == 'nq' and len(eval_dataset_reward) > 1 else data_source
            metric_dict[f'val/test_score/{metric_name}'] = np.mean(rewards)
        for eval_dataset, rewards in eval_dataset_reward.items():
            metric_dict[f'val/test_score_by_dataset/{eval_dataset}'] = np.mean(rewards)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Create one collision-free group id per source question before sampling
                # multiple trajectories. Dataset-local integer indices are not unique after
                # concatenating NQ, TriviaQA, HotpotQA, and 2Wiki.
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        batch = compute_advantage(batch,
                                                  ...
                                                  norm_adv_by_std_in_grpo=getattr(self.config.algorithm, 'norm_adv_by_std_in_grpo', True),
                                                  info_gain_norm_mode=getattr(self.config.algorithm, 'info_gain_norm_mode', 'joint'),
                                                  use_counterfactual_ig=getattr(self.config.algorithm, 'use_counterfactual_ig', False))
